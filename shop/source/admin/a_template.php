...
class a_template extends syController
{
	...
	function add(){
		...
		if($this->syArgs('run')==1){
			...
			if($db){
				...
				$this->msg='1：本模板需要覆盖数据，强烈建议先进行系统数据库备份操作<br>2：如果没有进行"备份数据"就安装，将造成现有数据丢失。<br>亲，我提醒过了喔~';
	...
	function export(){
		...
		if($this->syArgs('run')==1){
			$this->dbexport($t);
			message_a('备份成功，您可以将template/'.$t.'及skin/'.$t.'拷贝至其它LEY系统使用','?c='.$this->Get_c);
		}
		$this->msgtitle='确定要备份模板 <strong>['.$t.']</strong> 数据文件吗？';
		$this->msg='1：将导出当前系统数据，并生成模板安装数据库文件<br>2：该数据主要用于安装此模板时，测试前台页面预览效果<br>3：此操作不会导出系统隐私的数据(如管理员、会员信息等)<br>4：亲，请放心使用~';
