...
class syController { 
	...
	{	
		if(TRUE == $GLOBALS['G_QX']['view']['enabled']){
	...
	{
		if(TRUE == $GLOBALS['G_QX']['view']['enabled'] && false !== $value){
	...
	{
		if( TRUE != $this->v->displayed && FALSE != $GLOBALS['G_QX']['view']['auto_display']){
	...
	{
		if(in_array($name, $GLOBALS['G_QX']["auto_load_controller"])){
