...
class a_product extends syController
{
	function __construct(){
		//print_R("----------------22222--------------------<br />");
		//$colors=$this->syArgs('colors',2);//by hch
		//print_R("----------------33333--------------------<br />");
		//print_R($colors);
		//print_R("----------------123--------------------<br />");exit;
		//print_R("----------------33333--------------------<br />");exit;
		parent::__construct();

		$this->gopage=$this->syArgs('page',0,1);
		$this->molds = 'product';
		$this->sqldb=$GLOBALS['G_QX']['db']['prefix'].$this->molds;
		...
		if ($this->syArgs('run')==1){
			...
			//print_R("------------------222222------------------<br />");
				if(false == $newVerifier){
					$piccolors=$this->syArgs('piccolors',2);//by hch
					$litpics = $this->syArgs('litpics',2);//by hch
					$colors = $this->syArgs('colors',2);//by hch
					$sizes = $this->syArgs('sizes',2);//by hch
					$prices = $this->syArgs('prices',2);//by hch
					$nums = $this->syArgs('nums',2);//by hch
					$titles = $this->syArgs('titles',2);//by hch
					//插入相应的商品号
					...
					//插入相应颜色的图片
					if(count($piccolors)==count($piccolors)){
						for($i=0;$i<count($piccolors);$i++){
							syDB('product_attributepic')->create(array('aid'=>$addnewrow1,'color'=>$piccolors[$i],'litpic'=>$litpics[$i]));
						}
					}else{
						message_a("数据不一致");
					}
					//插入sku
					if(count($colors)==count($sizes) && count($colors)==count($prices) && count($colors)==count($nums) && count($colors)==count($titles)){
						for($i=0;$i<count($colors);$i++){
							syDB('product_attributeall')->create(array('aid'=>$addnewrow1,'color'=>$colors[$i],'size'=>$sizes[$i],'nums'=>$nums[$i],'price'=>$prices[$i],'title'=>$titles[$i]));
						}
					}else{
						message_a("数据不一致");
					}
					...
					if(syDB($this->molds.'_field')->create($this->newrow2)){
						if($GLOBALS['G_QX']['rewrite']["rewrite_open"]!=1&&syExt('site_html')==1&&$this->newrow1['isshow']==1&&$this->newrow1['mrank']==0&&$this->newrow1['mgold']==0){
							...
							$body=array_filter(explode("[ley|page]",$this->newrow2['body']));
							$allb=count($body);
							if($allb>1){
								for ($i = 1; $i <= $allb; $i++) {
									if($i>1){
						...
						deleteDir($GLOBALS['G_QX']['sp_cache']);
	...
	function edit(){
		$id=$this->syArgs('id');
		$sql="SELECT attrpic.*,attr.name FROM qx_attribute attr LEFT JOIN qx_product_attributepic attrpic ON attrpic.color =attr.sid where attrpic.aid=".$id;
		$this->pics=syDB("product_attributepic")->findSql($sql);
		//print_R($sql);exit;
		$sql="SELECT attrall.*,color.name as colorname,size.name as sizename FROM qx_product_attributeall attrall
				LEFT JOIN qx_attribute color  ON attrall.color =color.sid
				LEFT JOIN qx_attribute size  ON attrall.size =size.sid
				where attrall.aid=".$id;
		$this->inventorys=syDB("product_attributeall")->findSql($sql);
		//print_R($this->inventorys);exit;
		...
		if ($this->syArgs('run')==1){
			...
			$newVerifier=$this->ClassADD->syVerifier($this->newrow1);
				if(false == $newVerifier){
					...
					//$picids=$this->syArgs('picids',2);//by hch
					//$invids=$this->syArgs('invids',2);//by hch
					$piccolors=$this->syArgs('piccolors',2);//by hch
					$litpics = $this->syArgs('litpics',2);//by hch
					$colors = $this->syArgs('colors',2);//by hch
					$sizes = $this->syArgs('sizes',2);//by hch
					...
					//插入相应颜色的图片
					if(count($piccolors)==count($piccolors)){
						syDB('product_attributepic')->delete(array('aid'=>$id));
						for($i=0;$i<count($piccolors);$i++){
							syDB('product_attributepic')->create(array('aid'=>$id,'color'=>$piccolors[$i],'litpic'=>$litpics[$i]));
						}
					}else{
						message_a("数据不一致");
					}
					//插入sku
					if(count($colors)==count($sizes) && count($colors)==count($prices) && count($colors)==count($nums) && count($colors)==count($titles)){
						syDB('product_attributeall')->delete(array('aid'=>$id));
						for($i=0;$i<count($colors);$i++){
							syDB('product_attributeall')->create(array('aid'=>$id,'color'=>$colors[$i],'size'=>$sizes[$i],'nums'=>$nums[$i],'price'=>$prices[$i],'title'=>$titles[$i]));
						}
					}else{
						message_a("数据不一致");
					}
					...
					/*if(count($piccolors)==count($piccolors)){
						for($i=0;$i<count($piccolors);$i++){
							if(syDB('product_attributepic')->update(array('id'=>$picids[$i]),array('color'=>$piccolors[$i],'litpic'=>$litpics[$i]))==FALSE)
							{message_a("主表数据修改失败，请重新提交");}
						}
					}else{
						message_a("数据不一致");
					...
					if(count($colors)==count($sizes) && count($colors)==count($prices) && count($colors)==count($nums) && count($colors)==count($titles)){
						for($i=0;$i<count($colors);$i++){
							syDB('product_attributeall')->update(array('id'=>$invids[$i]),array('color'=>$colors[$i],'size'=>$sizes[$i],'nums'=>$nums[$i],'price'=>$prices[$i],'title'=>$titles[$i]));
						}
					}else{
						message_a("数据不一致");
					}*/
	...
	function del(){
		...
		if ($this->syArgs('run')==1){
			...
			if(syDB($this->molds)->delete(array('id'=>$id))&&syDB($this->molds.'_field')->delete(array('aid'=>$id))){
				...
				syDB($this->molds.'_discount')->delete(array('aid'=>$id));
				syDB($this->molds.'_attributeall')->delete(array('aid'=>$id));
				syDB($this->molds.'_attributepic')->delete(array('aid'=>$id));
		...
		switch ($this->syArgs('go')) {
			...
			case 2:
				foreach($ids as $tp){
					...
					if($this->auser->checkclass($a['tid'])){
						...
						syDB($this->molds.'_attributeall')->delete(array('aid'=>$tp));
						syDB($this->molds.'_attributepic')->delete(array('aid'=>$tp));
					}
				}
				break;
			case 3:
			...
			case 4:
				foreach($ids as $tp){
					$a=$this->ClassADD->find(array('id'=>$tp), null, " id,tid ");
					if($this->auser->checkclass($a['tid'])){
						$trait=','.$this->syArgs('trait').',';
						$this->ClassADD->update(array('id'=>$tp),array('trait'=>$trait));
					}
				}
				break;
			case 9:
