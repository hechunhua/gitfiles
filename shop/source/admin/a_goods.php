...
class a_goods extends syController
{
	...
	function index(){
		...
		if($w)$w=substr($w,3);
		$total_page=total_page($GLOBALS['G_QX']['db']['prefix'].'order');
	...
	function tadd(){
		if ($this->syArgs('run')==1){
			...
			if($this->newrow['biaoshi']=='') message_a("唯一标识不能为空");
	...
	function attribute_ajax(){
		...
		foreach($t as $v){
			//输出 颜色，尺码，并在编辑状态下选中
			$attr= !empty($v['biaoshi'])?$v['biaoshi']:message_a("规格未填写标识");
			...
			foreach($a as $vv){
				...
				$t.='<input group="'.$attr.'" name="attribute'.$v['tid'].'[]" type="checkbox" colorname="'.$vv['name'].'" value="'.$vv['sid'].'" '.$checked.' /> <strong>'.$vv['name'].'</strong>';//&nbsp;&nbsp;&nbsp;价格增减：<input name="aprice'.$vv['sid'].'" type="text" class="int" style="width:50px; height:12px;" value="'.$aprice.'" /> 元<br />

			}
			echo '<dl><dt>'.$v['name'].'：</dt><dd>'.$t.'</dd></dl>';
		}
	}
	function attribute_ajax_pic(){
		$id=$this->syArgs('id');
		if(!$this->syArgs('tid')){echo '<dl><dt>规格选项：</dt><dd>未选择栏目，无法加载规格选项，请先选择栏目。</dd></dl>';}
		echo '<table width="280" class="tablist">';
		echo '<tr><td width="85">颜色分类</td><td>图片</td></tr>';
		echo '</table>';
	}
	function logistics(){
		$this->toptxt='物流管理';
		$this->lists = $GLOBALS['G_QX']['logistics'];
		$this->display("goods.html");
	}
