...
class ajax extends syController
{
	function __construct(){
		...
		$this->db=$GLOBALS['G_QX']['db']['prefix'];
	}
	function vercode(){
		if(md5(strtolower($this->syArgs("vercode",1)))!=$_SESSION['ley_verify']){echo 'false';}else{echo 'true';}
	}
	function mycart(){
	...
	function comment(){
		if(funsinfo('comment','isshow')==1){
			$c=syClass('c_comment');
			$total_page=total_page($GLOBALS['G_QX']['db']['prefix'].'comment where isshow=1 and aid='.$this->syArgs('aid').' and molds="'.$this->syArgs('molds',1).'"');
			...
			$this->comment_page=pagetxt_ajax($c_page,$GLOBALS['G_QX']['url']["url_path_base"].'?c='.$this->syArgs('molds',1).'&id='.$this->syArgs('aid'),"ajax_comment('".$this->syArgs('id',1)."','".$this->syArgs('molds',1)."',".$this->syArgs('aid').",[_page_],'".$this->syArgs('template',1)."');");
			$this->display($this->syArgs('template',1));
		}
	}
	function record(){
		$c=syClass("c_sales_record");
		$total_page=total_page($GLOBALS['G_QX']['db']['prefix'].'sales_record where aid='.$this->syArgs('aid'));
		$this->record=$c->syPager($this->syArgs('record_page',0,1),10,$total_page)->findAll(array('aid'=>$this->syArgs('aid')),' stime desc ');
		$c_page=$c->syPager()->getPager();
		$this->record_page=pagetxt_ajax($c_page,$GLOBALS['G_QX']['url']["url_path_base"].'?c=product&id='.$this->syArgs('aid'),"ajax_record('".$this->syArgs('id',1)."',".$this->syArgs('aid').",[_page_],'".$this->syArgs('template',1)."');");
		$this->display($this->syArgs('template',1));
	}
}