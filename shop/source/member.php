...
{	
	function __construct(){
		...
		$this->dbl=$GLOBALS['G_QX']['db']['prefix'];
	...
	function login(){
		if($this->syArgs("go")==1){
			if($this->syArgs("user",1) && $this->syArgs("pass",1)){
				if($GLOBALS['G_QX']['vercode']==1){
				if(!$this->syArgs("vercode",1)||md5(strtolower($this->syArgs("vercode",1)))!=$_SESSION['ley_verify'])message("验证码错误");
				}
	...
	function reg(){
		...
		if($this->syArgs("go")==1){
			...
			if($this->syArgs('pass1',1)!=$this->syArgs('pass2',1))message("两次密码输入不一致");
			if($GLOBALS['G_QX']['vercode']==1){
			if(md5(strtolower($this->syArgs("vercode",1)))!=$_SESSION['ley_verify']){message("验证码错误");}
			}
			$newrow1 = array(
				'user' => $this->syArgs('user',1),
	...
	function mylist(){
		...
		$db=$GLOBALS['G_QX']['db']['prefix'].$this->type['molds'];
		...
		$this->pages=pagetxt($pages,$GLOBALS['G_QX']['url']["url_path_base"].'?c=member&a=mylist&tid='.$tid);
		$this->display("member/mylist.html");
	}
	function mymessage(){
		$c=syClass('c_message');
		$total_page=total_page($GLOBALS['G_QX']['db']['prefix'].'message where user="'.$this->my['user'].'"');
		...
		$this->pages=pagetxt($c_page,$GLOBALS['G_QX']['url']["url_path_base"].'?c=member&a=mymessage');
		$this->display("member/mymessage.html");
	}
	function mycomment(){
		$c=syClass('c_comment');
		$total_page=total_page($GLOBALS['G_QX']['db']['prefix'].'comment where user="'.$this->my['user'].'"');
		$this->lists=$c->syPager($this->syArgs('page',0,1),10,$total_page)->findAll(array('user'=>$this->my['user']),' addtime desc ');
		$c_page=$c->syPager()->getPager();
		$this->pages=pagetxt($c_page,$GLOBALS['G_QX']['url']["url_path_base"].'?c=member&a=mycomment');
		$this->display("member/mycomment.html");
	}
	function mymolds(){
		$c=syClass('c_account');
		$total_page=total_page($GLOBALS['G_QX']['db']['prefix'].'account where uid='.$this->my['id'].' and type=4');
		$this->lists = $c->syPager($this->syArgs('page',0,1),20,$total_page)->findAll(array('uid'=>$this->my['id'],'type'=>4),' addtime desc ');
		$pages=$c->syPager()->getPager();
		$this->pages=pagetxt($pages,$GLOBALS['G_QX']['url']["url_path_base"].'?c=member&a=mymolds');
	...
	function myorder(){
		if($this->syArgs('oid')||$this->syArgs('orderid',1)!=''){
			...
			$c=syClass('c_order');
			$total_page=total_page($GLOBALS['G_QX']['db']['prefix'].'order where uid='.$this->my['id']);
			$this->lists=$c->syPager($this->syArgs('page',0,1),10,$total_page)->findAll(array('uid'=>$this->my['id']),' addtime desc ');
			$c_page=$c->syPager()->getPager();
			$this->pages=pagetxt($c_page,$GLOBALS['G_QX']['url']["url_path_base"].'?c=member&a=myorder');
	...
	function account(){
		$a=syClass('syaccount');
		$c=syClass('c_account');
		$total_page=total_page($GLOBALS['G_QX']['db']['prefix'].'account where uid='.$this->my['id']);
		...
		$this->lists = $lists;
		$pages=$c->syPager()->getPager();
		$this->pages=pagetxt($pages,$GLOBALS['G_QX']['url']["url_path_base"].'?c=member&a=account');
