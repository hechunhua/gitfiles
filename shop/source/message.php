...
{	
	...
	function m_upload(){
		...
		if($this->my['id']!=0){
			$ufm=syDB('member_file')->findSql('SELECT sum(size) FROM '.$GLOBALS['G_QX']['db']['prefix'].'member_file where uid='.$this->my['id']);
			...
			$ufm=syDB('member_file')->findSql('SELECT sum(size) FROM '.$GLOBALS['G_QX']['db']['prefix'].'member_file where ip="'.$ip.'"');
