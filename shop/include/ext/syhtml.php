...
class syhtml
{
	...
	public function make($spurl, $alias_url = null, $todir = 1)
	{
		...
		@list($geturl, $controller, $action, $args, $anchor) = $spurl;
			if($todir!=0){
				$file_root_name = ( '' == $GLOBALS['G_QX']['ext']['site_html_dir'] ) ? '' : $GLOBALS['G_QX']['ext']['site_html_dir'].'/';
			}
			if( null == $alias_url ){
			...
			$baseuri = rtrim(dirname($GLOBALS['G_QX']['url']["url_path_base"]), '/\\')."/".$filedir.$filename;
		...
		if( $url_list = syAccess('r', 'sp_url_list') ){
			...
			foreach( $url_list as $url ){
				if( substr($url,0,strlen($url_input)) == $url_input ){
					...
					if( TRUE == $GLOBALS['G_QX']['html']['safe_check_file_exists'] && TRUE != $force_no_check ){
