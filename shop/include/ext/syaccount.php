			...
			case 3:
				...
				if(!$cz){
					...
					if($virtualtype['virtual']==1){
						...
						syDB('product_virtual')->runSql('update '.$GLOBALS['G_QX']['db']['prefix'].'product_virtual SET state=1,oid='.$virtualtype['id'].' WHERE aid = '.$vgoods[0]['aid'].' and state=0 order by id limit '.$quantity);
