...
class a_html extends syController
{
	...
	function clear(){
		...
		deleteDir($GLOBALS['G_QX']['view']['config']['template_tpl']);
		...
		if($t==1||$t==0){
			...
			foreach($m as $mv){
				...
				if($mv=='article'||$mv=='product'){
					$sql='select id,tid,addtime,htmlfile,body from '.$GLOBALS["G_QX"]["db"]["prefix"].$mv.' a left join '.$GLOBALS["G_QX"]["db"]["prefix"].$mv.'_field b on (a.id=b.aid) where isshow=1'.$ww.' and mrank=0 and mgold=0';
				}else{
					$sql='select id,tid,addtime,htmlfile from '.$GLOBALS["G_QX"]["db"]["prefix"].$mv.' a left join '.$GLOBALS["G_QX"]["db"]["prefix"].$mv.'_field b on (a.id=b.aid) where isshow=1'.$ww.' and mrank=0 and mgold=0';
				}
				$numall=syDB($mv)->findSql('select count(id) as ct from '.$GLOBALS["G_QX"]["db"]["prefix"].$mv.' where isshow=1'.$ww.' and mrank=0 and mgold=0');
	...
	private function chtml_molds($molds,$a,$anum,$isnum) {
		foreach($a as $v){
			...
			if($ms['sys']!=1){
				...
				$body=array_filter(explode("[ley|page]",$v['body']));
				$allb=count($body);
				if($allb>1){
					for ($i = 1; $i <= $allb; $i++) {
						if($i>1){
	...
	private function chtml_classtype($a) {
		...
		foreach($a as $v){
			...
			$total_page=total_page($GLOBALS['G_QX']['db']['prefix'].$v['molds'].' where isshow=1 and tid in('.$this->types->leafid($v['tid']).')');
	...
	private function chtml_special($a) {
		...
		foreach($a as $v){
			...
			$cl=syClass('c_'.$v['molds']);
			$total_page=total_page($GLOBALS['G_QX']['db']['prefix'].$v['molds']);
