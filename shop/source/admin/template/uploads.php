...
function doyocms_upload_config(){
	...
	var bgimage='include/js/leyupload/add.gif';
...
<?php if($fileover==1){
	...
	$(".info").empty();
	<?php } ?>
	$(".info").append('<li><p class="loading"><img src="include/js/leyupload/loading.gif" /></p><p class="file"><span>'+date+'</span><br /><span class="progress"></span></p><p class="cancel"><a onclick="doyocms_upload_gocancel('+"'"+date+"'"+',1);"></a></p></li>');
<?php } ?>
};
...
<?php }else{ ?>
	...
	if(tt.indexOf('0,')==-1){
		...
		$(window.parent.document).find("#img<?php echo $inputid ?>").attr("src",'<?php echo $GLOBALS['WWW'];?>'+strs[1]);
...
.list li .cancel a{float:left; width:28px; height:25px; cursor:pointer; background:url(include/js/leyupload/cancel.gif) no-repeat 0 5px;}
