<?php
if(!defined('APP_PATH')||!defined('LEY_PATH')){exit('Access Denied');}
function spRun(){
	...
	$handle_controller = syClass($__controller, null, $GLOBALS['G_QX']["controller_path"].'/'.$__controller.".php");
	if(!is_object($handle_controller) || !method_exists($handle_controller, $__action)){
		syError('route Error');
		exit;
	}
	$handle_controller->$__action();
	if(FALSE != $GLOBALS['G_QX']['view']['auto_display']){
		$__tplname = $__controller.$GLOBALS['G_QX']['view']['auto_display_sep'].
				$__action.$GLOBALS['G_QX']['view']['auto_display_suffix']; 
...
function dump($vars, $output = TRUE, $show_trace = FALSE){
	if(TRUE != SP_DEBUG && TRUE != $GLOBALS['G_QX']['allow_trace_onrelease'])return;
	if( TRUE == $show_trace ){ 
...
function import($sfilename, $auto_search = TRUE, $auto_error = FALSE){
	if(isset($GLOBALS['G_QX']["import_file"][md5($sfilename)]))return TRUE;
	if( TRUE == @is_readable($sfilename) ){
		require($sfilename); 
		$GLOBALS['G_QX']['import_file'][md5($sfilename)] = TRUE; 
		return TRUE;
	}else{
		if(TRUE == $auto_search){
			foreach(array_merge( $GLOBALS['G_QX']['include_path'], array($GLOBALS['G_QX']['model_path']), $GLOBALS['G_QX']['sp_include_path'] ) as $sp_include_path){
				if(isset($GLOBALS['G_QX']["import_file"][md5($sp_include_path.'/'.$sfilename)]))return TRUE;
				if( is_readable( $sp_include_path.'/'.$sfilename ) ){
					require($sp_include_path.'/'.$sfilename);
					$GLOBALS['G_QX']['import_file'][md5($sp_include_path.'/'.$sfilename)] = TRUE;
					return TRUE;
				}
			}
		}
...
function syAccess($method, $name, $value = NULL, $life_time = -1){
	...
	if(!is_dir($GLOBALS['G_QX']['sp_cache']))__mkdirs($GLOBALS['G_QX']['sp_cache']);
	$sfile = $GLOBALS['G_QX']['sp_cache'].'/'.$GLOBALS['G_QX']['sp_app_id'].md5($name).".php";
	if('w' == $method){ 
...
function syClass($class_name, $args = null, $sdir = null, $force_inst = FALSE){
	...
	if(TRUE != $force_inst)if(isset($GLOBALS['G_QX']["inst_class"][$class_name]))return $GLOBALS['G_QX']["inst_class"][$class_name];
	...
	if(FALSE != $has_define){
		...
		eval("\$GLOBALS['G_QX']['inst_class'][\$class_name]= new \$class_name($argString);"); 
		return $GLOBALS['G_QX']["inst_class"][$class_name];
	}
...
function syError($msg, $output = TRUE, $stop = TRUE){
	if($GLOBALS['G_QX']['sp_error_throw_exception'])throw new Exception($msg);
	if(TRUE != SP_DEBUG){
		//error_log($msg);
	...
	require_once($GLOBALS['G_QX']['notice_php']);
	if(TRUE == $stop)exit;
}


function spLaunch($configname, $launchargs = null, $returns = FALSE ){
	if( isset($GLOBALS['G_QX']['launch'][$configname]) && is_array($GLOBALS['G_QX']['launch'][$configname]) ){
		foreach( $GLOBALS['G_QX']['launch'][$configname] as $launch ){
			if( is_array($launch) ){
...
function spUrl($geturl = null, $controller = null, $action = null, $args = null, $anchor = null, $no_sphtml = FALSE) {
	if(TRUE == $GLOBALS['G_QX']['html']["enabled"] && TRUE != $no_sphtml){
	...
	$controller = ( null != $controller ) ? $controller : $GLOBALS['G_QX']["default_controller"];
	$action = ( null != $action ) ? $action : $GLOBALS['G_QX']["default_action"];
	...
	if( TRUE == $GLOBALS['G_QX']['url']["url_path_info"] ){
		$url = "{$controller}/{$controller}/{$action}";
		if(null != $args)foreach($args as $key => $arg) $url .= "/{$key}/{$arg}";
	}else{
		$url = $geturl."?". $GLOBALS['G_QX']["url_controller"]. "={$controller}&";
		$url .= $GLOBALS['G_QX']["url_action"]. "={$action}";
		if(null != $args)foreach($args as $key => $arg) $url .= "&{$key}={$arg}";
	}
...
{
	return (empty($GLOBALS['G_QX']['ext'][$ext_node_name])) ? FALSE : $GLOBALS['G_QX']['ext'][$ext_node_name];
}
function syCus($ext_node_name)
{
	return (empty($GLOBALS['G_QX']['cus'][$ext_node_name])) ? FALSE : $GLOBALS['G_QX']['cus'][$ext_node_name];
}
function spAddViewFunction($alias, $callback_function)
{
	return $GLOBALS['G_QX']["view_registered_functions"][$alias] = $callback_function;
}

function syDB($tbl_name, $pk = null){
	$modelObj = syClass("syModel");
	$modelObj->tbl_name = (TRUE == $GLOBALS['G_QX']["db_spdb_full_tblname"]) ? $tbl_name :	$GLOBALS['G_QX']['db']['prefix'] . $tbl_name;
	if( !$pk ){
...
function dykeycode($string, $operation = 'DECODE'){
	$ckey_length = 4;
	$key = md5($GLOBALS['G_QX']['ext']['secret_key'] != '' ? $GLOBALS['G_QX']['ext']['secret_key'] : 'ley_secret_key');
...
//统计
function  total_page($sql,$v='ley_total_page'){
...
function order_goods($d,$logistics){
	foreach($d as $k=>$v){
		...
		$p_type=syDB('attribute_type')->findSql('select distinct a.tid,a.aid,b.tid,b.isshow,b.orders,b.name from '.$GLOBALS['G_QX']['db']['prefix'].'product_attribute a left join '.$GLOBALS['G_QX']['db']['prefix'].'attribute_type b on (a.tid=b.tid) where a.aid='.$v['aid'].' and b.isshow=1 order by b.orders desc,b.tid desc');
...
function html_url($type,$c,$pages=0,$ispage,$molds){
	...
	$sh=syExt("site_html");
	$sr=$GLOBALS['G_QX']['url']["url_path_base"];
	$sg=$GLOBALS["WWW"];
	$re=$GLOBALS['G_QX']['rewrite']["rewrite_open"];
	switch($type){
		case 'channel':
			if($re==1){
				$re_url=$sg.$GLOBALS['G_QX']['rewrite']["rewrite_channel"];
		...
		case 'article':
			if($re==1){
				$re_url=$sg.$GLOBALS['G_QX']['rewrite']["rewrite_article"];
				if($c['htmlfile']=='')$c['htmlfile']=$c['id'];
		...
		case 'product':
			if($re==1){
				$re_url=$sg.$GLOBALS['G_QX']['rewrite']["rewrite_product"];
				if($c['htmlfile']=='')$c['htmlfile']=$c['id'];
				$go_url=str_replace(array('{id}','{file}'),array($c['id'],$c['htmlfile']),$re_url);
		...
		case 'classtype':
			if($re==1){
				if($c["molds"]!='article'&&$c["molds"]!='product'&&$c["molds"]!='message'){
					$re_url=$sg.$GLOBALS['G_QX']['rewrite']['rewrite_channel_type'];
				}else{
					$re_url=$sg.$GLOBALS['G_QX']['rewrite']['rewrite_'.$c['molds'].'_type'];
				}
		...
		case 'special':
			if($re==1){
				$re_url=$sg.$GLOBALS['G_QX']['rewrite']["rewrite_special"];
		...
		case 'labelcus_custom':
			if($re==1){
				$re_url=$sg.$GLOBALS['G_QX']['rewrite']["rewrite_labelcus_custom"];
...
function pagetxt_html($url,$total_page,$current_page,$pageno=3){
	if($GLOBALS['G_QX']['rewrite']["rewrite_open"]==1){$is_p=1;}else{$is_p='';}
	$pagetxt='';
