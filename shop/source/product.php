...
class product extends syController
{
	function __construct(){
		...
		$this->db=$GLOBALS['G_QX']['db']['prefix'].'product';
	}
	function index(){
		...
		$this->attribute_type=syDB('product')->findSql('select distinct a.tid,a.aid,b.tid,b.isshow,b.orders,b.name from '.$this->db.'_attribute a left join '.$GLOBALS['G_QX']['db']['prefix'].'attribute_type b on (a.tid=b.tid and a.aid='.$this->product['id'].') where b.isshow=1 order by b.orders desc,b.tid desc');
		...
		$this->record=total_page($GLOBALS['G_QX']['db']['prefix'].'sales_record where aid='.$this->product['id']);
		$body=array_filter(explode("[ley|page]",$this->product['body']));
		if(count($body)>1){
			$pages=array(
