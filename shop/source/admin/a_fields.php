...
class a_fields extends syController
{
	...
	function index(){
		...
		$total_page=total_page($GLOBALS['G_QX']['db']['prefix'].'fields where'.$w);
	...
	function info(){
		...
		foreach($v as $f){
			switch ($f['fieldstype']){
				...
				break;
				case 'text':
					$t='<script type="text/javascript">$(function(){KindEditor.create("#'.$f['fields'].'",{cssPath : ["include/js/prettify.css"],fileManagerJson : "'.$GLOBALS['G_QX']['url']["url_path_base"].'?c=uploads&a=filemanager",allowFileManager : true,filePostName : "editor_KindEditor",filterMode : false,uploadJson : "'.$GLOBALS['G_QX']['url']["url_path_base"].'?c=uploads&tid='.$this->tid.'&isfiles=editor_KindEditor"})});</script>';
