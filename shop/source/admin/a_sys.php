...
class a_sys extends syController
{
	...
	function index(){
		if($this->syArgs('run')==1){
			...
			foreach($config as $v){
				if (strpos(',site_html,cache_auto,cache_time,filesize,imgwater,imgwater_t,imgcaling,img_w,img_h,comment_audit,comment_user,site_html_index,enable_gzip,enable_gzip_level,',$v)){
					...
					if($v=='site_html'){
						if($GLOBALS['G_QX']['ext']['site_html']==1&&$txt==0)@unlink('index.html');
					}
		...
		$this->toptxt='系统设置';
		$this->d=$GLOBALS['G_QX']['ext'];
		$this->r=$GLOBALS['G_QX']['rewrite'];
		$this->inc=$GLOBALS['G_QX'];
	...
	function ecache(){
		$this->toptxt='清空缓存';
		if($this->syArgs('run')==1){
			if($this->syArgs("tmp")==1)deleteDir($GLOBALS['G_QX']['sp_cache']);
			if($this->syArgs("tpl")==1)deleteDir($GLOBALS['G_QX']['view']['config']['template_tpl']);
